	...
	def to_src_nodes(lst):
		...
		if isinstance(lst, Node.Node):
			...
			if not os.path.isfile(lst.abspath()):
				raise Errors.WafError(
					f'buildcopy: Cannot copy directory {lst} (unsupported action)'
				)
			return lst
		...
		if node:
			if not os.path.isfile(node.abspath()):
				raise Errors.WafError(
					f'buildcopy: Cannot copy directory {node} (unsupported action)'
				)
			return node
		...
		if node:
			...
			return node
		raise Errors.WafError(
			f'buildcopy: File not found in src: {os.path.join(*lst)}'
		)
